# maxinger15-music MCP Server

MCP (Model Context Protocol) server for the maxinger15-music plugin.
...
The server is registered as `maxinger15-music-mcp` in Codex. Future MCP tools can be added to this same server.
...
~/.maxinger15-music/venv/bin/pip install -r requirements.txt          # Mastering
~/.maxinger15-music/venv/bin/pip install -r requirements.txt    # Cloud uploads
~/.maxinger15-music/venv/bin/pip install playwright                   # Document hunter
~/.maxinger15-music/venv/bin/playwright install chromium
```

The MCP server automatically detects and uses `~/.maxinger15-music/venv` if it exists. No manual configuration needed.
...
After installing, **restart Codex** to reload the plugin.
...
```bash
# From plugin root
python3 servers/maxinger15-music-server/server.py
```
...
```
Codex → MCP Protocol → server.py → StateCache → indexer.py → state.json
```
