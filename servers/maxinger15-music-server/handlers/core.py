...
async def get_python_command() -> str:
    ...
    Returns:
        JSON with:
            python: Absolute path to ~/.maxinger15-music/venv/bin/python3
    ...
    if not venv_exists:
        result["warning"] = (
            ...
            "Create it with: python3 -m venv ~/.maxinger15-music/venv && "
            "~/.maxinger15-music/venv/bin/pip install pyloudnorm scipy numpy "
...
async def search(query: str, scope: str = "all") -> str:
    ...
    if scope in ("all", "skills"):
        ...
        for name, skill in state.get("skills", {}).get("items", {}).items():
            ...
            complexity_tier = skill.get("complexity_tier", "")
            if (query_lower in name.lower() or
                    query_lower in description.lower() or
                    query_lower in complexity_tier.lower()):
