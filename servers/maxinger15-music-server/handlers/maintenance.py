...
async def cleanup_legacy_venvs(
    ...
    """Detect and remove stale per-tool virtual environments from ~/.maxinger15-music/.

    Prior to 0.40.0, each tool had its own venv (mastering-env, promotion-env,
    cloud-env). These are now consolidated into a single ~/.maxinger15-music/venv/.
    ...
    """
    tools_root = Path.home() / ".maxinger15-music"
    results: dict[str, dict[str, Any]] = {}
    ...
    return _safe_json({
        ...
        "results": results,
        "note": "All tools now use ~/.maxinger15-music/venv/ (unified venv).",
    })
