...
def _check_cloud_enabled() -> str | None:
    ...
    try:
        ...
        return (
            "Could not load config. Ensure ~/.maxinger15-music/config.yaml exists."
        )
    if not config:
        return "Config not found. Run $maxinger15-music:configure first."
    cloud_config = config.get("cloud", {})
    if not cloud_config.get("enabled", False):
        return (
            "Cloud uploads not enabled. "
            "Set cloud.enabled: true in ~/.maxinger15-music/config.yaml. "
