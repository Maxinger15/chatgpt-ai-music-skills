#!/usr/bin/env python3
"""
MCP server for maxinger15-music plugin.
...
PLUGIN_ROOT = Path(
    os.environ.get("MAXINGER15_MUSIC_PLUGIN_ROOT") or
...
try:
    ...
    print("The maxinger15-music MCP server requires the MCP SDK.", file=sys.stderr)
    print("", file=sys.stderr)
    ...
    print("     python3 -m venv ~/.maxinger15-music/venv", file=sys.stderr)
    print("     ~/.maxinger15-music/venv/bin/pip install 'mcp[cli]>=1.2.0' pyyaml", file=sys.stderr)
    print("", file=sys.stderr)
    print("After installing, restart Codex to reload the plugin.", file=sys.stderr)
    print("=" * 70, file=sys.stderr)
    sys.exit(1)
...
mcp = FastMCP("maxinger15-music-mcp")
...
def main() -> None:
    ...
    logger.info("Starting maxinger15-music-state MCP server")
