...
You help users set up and manage their `~/.maxinger15-music/config.yaml` configuration.
...
1. Check if `~/.maxinger15-music/config.yaml` exists
2. If exists, ask if they want to overwrite or edit instead
3. If creating new:
   - Create `~/.maxinger15-music/` directory if needed
...
>   - AGENTS.md (custom workflow instructions)
...
> This is managed by the $maxinger15-music:album-ideas skill for brainstorming and planning.
>
...
**Example interaction:**
```
Let's set up your maxinger15-music configuration.
...
Creating config at ~/.maxinger15-music/config.yaml...
...
**Example output:**
```
maxinger15-music Configuration
Location: ~/.maxinger15-music/config.yaml
...
**Example output:**
```
Validating ~/.maxinger15-music/config.yaml...
...
3. Delete `~/.maxinger15-music/config.yaml`
4. Ask if they want to run setup now

**Example:**
```
⚠️  This will delete your configuration at ~/.maxinger15-music/config.yaml
...
If yes:
```
✓ Backed up to ~/.maxinger15-music/config.yaml.bak
✓ Deleted ~/.maxinger15-music/config.yaml
...
```yaml
# maxinger15-music Plugin Configuration
...
- **Preserve exact casing** - If user says "maxinger15", write "maxinger15" not "Maxinger15"
