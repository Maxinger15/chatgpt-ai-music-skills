...
- **Fix**: Run `$maxinger15-music:verify-sources [album]` to walk through human source verification for pending tracks.
...
- **Fix**: Run `$maxinger15-music:lyric-writer [track]` to write or complete the lyrics.
- **Severity**: BLOCKING
...
- **Fix**: Run `$maxinger15-music:pronunciation-specialist [track]` to scan and resolve pronunciation risks.
...
- **Fix**: Style Box is created by suno-engineer, which is normally auto-invoked by lyric-writer. Run `$maxinger15-music:suno-engineer [track]` to create the missing Style Box.
- **Severity**: BLOCKING

### Gate 6: Artist Names Cleared
- **Check**: Style prompt does not contain real artist/band names
- **Reference**: `{plugin_root}/reference/suno/artist-blocklist.md`
- **Fail if**: Any blocked artist name found in style prompt
- **Fix**: Run `$maxinger15-music:suno-engineer [track]` to regenerate the Style Box without artist names, or manually edit the Style Box to replace artist names with genre/style descriptors.
